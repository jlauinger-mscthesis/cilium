...
var _ = Describe("K8sConformance", func() {
	...
	BeforeAll(func() {
		...
		connectivityCheckYamlSimple = kubectl.GetFilePath("../examples/kubernetes/connectivity-check/connectivity-check-simple.yaml")
	...
	Context("Portmap Chaining", func() {
		...
		It("Check one node connectivity-check compliance with portmap chaining", func() {
			...
			kubectl.ApplyDefault(connectivityCheckYamlSimple).ExpectSuccess("cannot install connectivity-check-simple")
