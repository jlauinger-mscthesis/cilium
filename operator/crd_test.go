...
func (s *crdTestSuite) TestGetCRD(c *C) {
	...
	err = getCRD(client, "foo")
	c.Assert(err, IsNil)
	err = WaitForCRD(client, "foo")
	c.Assert(err, IsNil)

	// Try to get non-existing CRD
	err = getCRD(client, "bar")
	c.Assert(err, NotNil)
	err = waitForCRD(client, "bar", 1*time.Millisecond, 20*time.Millisecond)
	c.Assert(err, ErrorMatches, ".*timeout exceeded for successful run of controller wait-for-crd-bar")
}
